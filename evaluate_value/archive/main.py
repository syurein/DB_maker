...
class AI_Filter_Estimator:
    ...
    def estimate_final_price(self, target_name: str, filtered_records: list, stats_res: dict) -> dict:
        """
        Phase 2: フィルタ済みデータ、統計結果、そしてGoogle検索を元に、最終価格を決める
        """
        ...
        prompt = f"""
        あなたはプロの鑑定士です。
        提供された「参照市場データ」と「統計データ」を分析し、さらにGoogle検索を使用して現在の市場動向を確認した上で、最終的な買取/販売想定価格を決定してください。
        ...
        【指示】
        1. **Google検索を活用してください**: 商品の現在の相場観、再販情報、人気度を検索し、判断の裏付けにしてください。
        2. 「参照市場データ」をベースにしつつ、検索で見つかった最新情報で補正をかけてください。
        3. 最終的に「ひとつ」の価格を決定してください。

        【出力(JSON)】
        {{
            "final_ai_price": 5000,
            "reasoning": "市場データの中央値はX円ですが、Google検索によると現在需要が高騰しているため、相場より高めに設定しました..."
        }}
        """

        # Google Search Toolの設定
        # ※ SDKのバージョンによって記述が若干異なる場合がありますが、標準的な google-genai / vertex ai SDK を想定しています
        ...
        try:
            ...
            # エラー時はログを出してデフォルト動作（検索なしなど）にフォールバックすることも検討
